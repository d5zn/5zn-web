...
### 3. 🔑 Отсутствует обмен OAuth кода на токен
**Файл**: `oauth/index.html:80-100`

**Проблема**: Код пытается вызвать `/api/strava/token`, но этот endpoint не реализован на сервере.
...
**Решение**: Реализовать серверный endpoint для обмена кода на токен (это должно быть на backend, а не на frontend для безопасности).
